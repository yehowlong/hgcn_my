...
class NCModel(BaseModel):
    ...
    def __init__(self, args):
        ...
        # 修复 Bug：这里不能直接用 data['labels']，因为 data 没有传进来
        # 我们默认正负样本权重一致，或者在外部 train.py 里单独传进来
    ...
    def compute_metrics(self, embeddings, data, split):
        ...
        # 将 labels 确保为 LongTensor 类型，以适应 nll_loss
        ...
        # 确保 weights 和 labels 在同一个设备上
        weights = self.weights.to(output.device)

        loss = F.nll_loss(output, labels, weights)
